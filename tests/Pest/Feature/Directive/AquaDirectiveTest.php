...
test('the aqua directive works only if the _aquaDrips variable is available to the view', function() {
    expect(fn() => (string) view('aquatest::aqua-directive')->render())
...
test('the aqua directive wants the _aquaDrips be an array with needed keys', function() {
    expect(fn() => (string) view('aquatest::aqua-directive', ['_aquaDrips' => null])->render())
    ->toThrow(ViewException::class, 'Aquastrap missing drips');

    expect(fn() => (string) view('aquatest::aqua-directive', ['_aquaDrips' => ['pro', 'gram']])->render())
    ->toThrow(ViewException::class, 'Aquastrap detected malformed drips');

    expect(fn() => (string) view('aquatest::aqua-directive', ['_aquaDrips' => ['id' => 'shiv', 'key' => 'shambhu', 'ingredient' => 'shankar', 'methods' => ['mahakal', 'nagendra']]])->render())
...
test('the aqua directive prints _aquaGenerate javascript function to the page with proper parameters', function() {
    $content = (string) view('aquatest::aqua-directive', [
        '_aquaDrips' => [
            'id' => 'shiv', 
            'key' => 'shambhu', 
            'ingredient' => 'shankar', 
...
test('the aquaConfig directive wants the _aquaDrips be an array having id as key', function() {
    expect(fn() => (string) view('aquatest::aqua-config-directive', ['_aquaDrips' => null])->render())
    ->toThrow(ViewException::class, 'Aquastrap missing drips');

    expect(fn() => (string) view('aquatest::aqua-config-directive', ['_aquaDrips' => ['pro']])->render())
    ->toThrow(ViewException::class, 'Aquastrap detected malformed drips');

    expect(fn() => (string) view('aquatest::aqua-config-directive', ['_aquaDrips' => ['id' => 'shiv']])->render())
    ->not->toThrow(ViewException::class);
});

test('the aquaConfig directive prints _registerAquaConfig javascript function to the page with proper parameters', function() {
    $content = (string) view('aquatest::aqua-config-directive', [
        '_aquaDrips' => [
            'id' => 'shiv', 
            'key' => 'doesntmatter', 
